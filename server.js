 ...
 * 学员进度管理 - 飞书多维表格同步服务
 * 作用：
 *  1. 托管网页（http://localhost:3210）
 *  2. 代理飞书开放平台 API（浏览器无法直连，且避免暴露 app_secret）
 *  3. 首次运行自动在多维表格中创建「学员 / 职位 / 投递」三张数据表
 * 运行：node server.js （建议直接双击「启动服务.command」）
 */
const fs = require('fs');
...
let config = {};
try { config = JSON.parse(fs.readFileSync(path.join(DIR, 'config.json'), 'utf8')); } catch (e) {}
const PORT = process.env.PORT || config.port || 3210;
const ACCESS_PASSWORD = process.env.ACCESS_PASSWORD || config.access_password || '';
...
function isAuthed(req) {
  ...
  if (sid && sessions.has(sid)) {
    ...
    if (s.exp > Date.now()) return true;
    sessions.delete(sid);
  }
  return false;
}
function cookieStr(req, sid) {
  let c = `sid=${sid}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${Math.floor(SESSION_TTL / 1000)}`;
...
async function getToken() {
  ...
  if (j.code !== 0) throw new Error('获取飞书凭证失败：' + j.msg + '（请检查 app_id / app_secret）');
...
async function feishu(method, url, body) {
  ...
  if (j.code !== 0) {
    let hint = '';
    if (j.code === 91402 || /NOTEXIST/i.test(j.msg)) hint = '（app_token 不对，或还没把应用添加为多维表格的文档应用，见配置指南第 4 步）';
...
async function ensureTables() {
  ...
  for (const name of Object.keys(TABLE_DEFS)) {
    ...
    ids[name] = c.table_id;
    console.log(`[初始化] 已在飞书多维表格中创建数据表「${name}」`);
  }
  tableIds = ids;
  return ids;
}
  ...
  try {
    if (url.pathname.startsWith('/api/')) {
      ...
      if (url.pathname === '/api/login' && req.method === 'POST') {
        ...
        if (!AUTH_ENABLED) return json(res, { ok: false, error: '服务端未设置访问口令（请在 config.json 填写 access_password）' });
        if (b.password === ACCESS_PASSWORD) {
          const sid = newSid();
          sessions.set(sid, { exp: Date.now() + SESSION_TTL });
          res.setHeader('Set-Cookie', cookieStr(req, sid));
      ...
      if (!configured()) return json(res, { ok: false, error: '尚未配置飞书应用：请填写 config.json 中的 app_id / app_secret / app_token（详见《飞书配置指南.md》）' });
      if (AUTH_ENABLED && !isAuthed(req)) return json(res, { ok: false, error: '未登录或登录已过期', needLogin: true }, 401);
      if (url.pathname === '/api/data' && req.method === 'GET') return json(res, { ok: true, data: await getData() });
      if (url.pathname === '/api/save' && req.method === 'POST') { await saveEntity(await readBody(req)); return json(res, { ok: true }); }
      if (url.pathname === '/api/delete' && req.method === 'POST') { await deleteEntity(await readBody(req)); return json(res, { ok: true }); }
      if (url.pathname === '/api/migrate' && req.method === 'POST') return json(res, { ok: true, result: await migrate(await readBody(req)) });
    ...
    if (url.pathname.startsWith('/api/')) return json(res, { ok: false, error: e.message });
...
server.listen(PORT, HOST, () => {
  console.log('==============================================');
  console.log('  学员进度管理 · 同步服务已启动');
  console.log(`  本机访问： http://localhost:${PORT}`);
  console.log(`  飞书配置状态： ${configured() ? '✅ 已配置，将同步飞书多维表格' : '⚠️ 未配置（本地模式），请编辑 config.json'}`);
  console.log(`  访问口令： ${AUTH_ENABLED ? '✅ 已启用（公网/局域网访问需口令）' : '⚠️ 未设置（仅本机 localhost 可访问）'}`);
  console.log(`  监听地址： ${HOST}${AUTH_ENABLED ? '（已对外网/局域网开放，请务必保管好访问口令）' : '（仅本机）'}`);
  console.log('==============================================');
});
